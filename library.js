...
socketPlugins.imageMentions.keywordSearch = async function (socket, data) {
	...
	matches.forEach((match) => {
		...
		splitContent = splitContent.map((c, i) => {
			...
			return c.replace(matchRegex, function (inlineMatch) {
				...
				return `${plain}<img class="plugin-image-mentions-keyword plugin-image-mentions-img" src="${pair.link}" alt="${pair.keyword}" />`;
			});
		});
	});
